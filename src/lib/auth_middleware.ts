...
if (!process.env.ENCRYPTION_KEY) {
  console.warn(
    'No encryption key specified. A random key has been generated, but all users will need to log in again every time the app restarts.'
  );
}
...
export const decrypt = (dataBuffer: Buffer) => {
  ...
  return Buffer.concat([decipher.update(dataBuffer.slice(ivSize + 17)), decipher.final()]).toString(
    'utf-8'
  );
};
...
export const validateMagicLink = async (magiclink: string) => {
  try {
    ...
    if (await doesFileExist('magiclinks/' + magiclink + '-temp')) {
      tokenType = 'temp';
